...
## DINOv3 ViT-S16 LVD-1689M Image Embedding — 2026-05-22

- **Source**: https://huggingface.co/facebook/dinov3-vits16-pretrain-lvd1689m
- **Artifact ID**: Not yet submitted (validated locally)
- **Weight format**: `pytorch_state_dict` plus secondary `onnx` with `parent: pytorch_state_dict`
- **Framework version**: PyTorch 2.11.0, ONNX opset 17
- **Input shape**: `[1, 3, 224, 224]` RGB values in `[0, 1]`
- **Output shape**: `[1, 384]` pooled DINOv3 embedding

### Key challenges

1. **The Hugging Face model is gated** — download and pipeline generation require an authenticated token with access to the model.

2. **The upstream `dinov3-license` is not SPDX** — model spec 0.5.11 supports custom license files, so the RDF uses:
   ```yaml
   license:
     source: LICENSE.md
     sha256: <hash>
...
3. **Hugging Face pipeline preprocessing differed from the RDF preprocessing** — the HF image processor applied fixed ImageNet mean/std normalization. The RDF used dynamic `zero_mean_unit_variance` over spatial axes, so `test_output.npy` was generated by instantiating the HF `image-feature-extraction` pipeline but feeding `pipe.model(pixel_values=...)` with tensors preprocessed according to the RDF.

4. **ONNX Runtime provider drift on macOS** — CPUExecutionProvider reproduced the test tensor, while the default CoreMLExecutionProvider showed embedding drift large enough to fail strict element-wise comparison. Fix: validate ONNX on CPU when checking package correctness and record provider-specific drift separately.

5. **Generated artifacts should stay out of source control** — keep `build_package.py`, source architecture, and packaging notes in the package root; write the actual BioImage.IO package into `generated/` and add that folder to `.gitignore`.

### What worked

- A self-contained `dinov3_vit.py` architecture using only `torch`, `torch.nn`, `torch.nn.functional`, and `math`
- A pure Hugging Face model `state_dict` saved as `weights.pt`
- ONNX exported from the same model wrapper with `dynamo=False` and opset 17
- `packaged_by` used to distinguish Meta AI model authors from the BioImage.IO packager
- Static validation and dynamic `bioimageio test` passed from `generated/rdf.yaml`

### rdf.yaml snippets

Custom license file and generated-file descriptors:
...
Dynamic preprocessing and parented ONNX weights:
...
weights:
  pytorch_state_dict:
    source: weights.pt
    architecture:
      source: dinov3_vit.py
      callable: DINOv3ViTS16
  onnx:
    source: weights.onnx
    opset_version: 17
...
3. **Legacy note for spec 0.5.9: `covers` were plain string paths** — this older run used spec 0.5.9, where the covers field accepted string paths. In current model spec 0.5.11+, use file descriptors with `source` and `sha256`.
   ```yaml
   covers:
     - source: cover.png
       sha256: <hash>
...
- Static validation (`bioimageio.spec 0.5.9.1`): passed on 2nd attempt after fixing legacy covers + SizeReference
...
- **Resolved in current skill**: current spec guidance uses file descriptors for `covers` and `documentation`.
