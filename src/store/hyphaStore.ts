...
export const useHyphaStore = create<HyphaState>((set, get) => ({
  ...
  fetchResources: async (page: number, searchQuery?: string, filterOptions?: FilterOptions) => {
    ...
    try {
      ...
      let url = `https://hypha.aicell.io/ri-scale/artifacts/bioimage.io/children?pagination=true&offset=${offset}&limit=${get().itemsPerPage}&stage=false&order_by=manifest.score>`;
