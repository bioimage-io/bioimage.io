...
import { withStageRetry } from './datasetApi';
  ...
  useEffect(() => {
    ...
    const loadImageUrls = async () => {
      try {
        ...
        if (imageInArtifact) {
          try {
            const rawUrl = await withStageRetry(() =>
              artifactManager.get_file({
                artifact_id: dataArtifactId,
                file_path: `${imageFolder}/${baseName}.png`,
                stage: true,
                _rkwargs: true,
              }),
            );
            ...
            console.error('Failed to get presigned image URL:', error);
            setImageUrl('');
          }
        } else {
        ...
        if (annotated) {
          ...
          try {
            const annUrl = await withStageRetry(() =>
              artifactManager.get_file({
                artifact_id: dataArtifactId,
                file_path: `${maskFolder}/${relPath}`,
                stage: true,
                _rkwargs: true,
              }),
            );
            ...
            console.error('Failed to get presigned annotation URL:', error);
            setAnnotationUrl('');
          }
        } else {
          setAnnotationUrl('');
    ...
    loadImageUrls();
  }, [selectedImage, dataArtifactId, label, artifactManager, annotationsList, dataSourceType, imageFolderHandle, executeCode, annotationURL, server, cloudImages, getImageFolder]);
