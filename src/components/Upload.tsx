...
const Upload: React.FC<UploadProps> = ({ artifactId }) => {
  ...
  const handleAgreeAndUpload = async () => {
    try {
      ...
      for (let index = 0; index < remainingFiles.length; index++) {
        ...
        // test_report.json is generated by the CI pipeline and must not be
        // uploaded manually — it would be overwritten on the next test run.
        if (file.name === 'test_report.json') {
          console.log(`Skipping upload of ${file.path} (test_report.json is managed by CI)`);
          autoSkippedFiles.push(file.name);
          continue;
        }
      ...
      // If any files were silently skipped, show a brief warning first.
      if (autoSkippedFiles.length > 0) {
        setUploadStatus({
          message: `Skipped ${autoSkippedFiles.join(', ')} — this file is managed by the CI pipeline and cannot be uploaded manually.`,
          severity: 'warning',
        });
  ...
  const getStatusColor = (severity: 'info' | 'success' | 'error' | 'warning') => {
    switch (severity) {
      case 'info':
              ...
              <div className="p-2">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                  ...
                  <div className="flex gap-2 flex-shrink-0 items-center">
                    {/* Advanced Options popover. Sits before Validate so the
                        submitter can point ModelValidator at a private
                        BioEngine (Server URL + Service ID override) or pick
