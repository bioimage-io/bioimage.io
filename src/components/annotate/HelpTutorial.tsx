...
const STEPS: TutorialStep[] = [
  ...
  { text: 'Expand Mask (A): paint a freehand area to add it to the nearest mask it touches. The added area is clipped at the image edges automatically, so you can paint loosely without checking the border yourself. It also shares the lasso/brush mode toggled from Draw Mask.', highlightSelector: '[data-tool="expander"]' },
  ...
  { text: 'Interactive Segmentation (B): draw a box around one cell for a single μSAM-generated mask. Open its gear icon once to pick a model and prepare it, downloading the decoder and computing this image\'s embedding. "Start annotating" unlocks once both are ready, and B then activates the tool directly from then on. If you press B or click the tool before it is ready, this same dialog opens instead. Like Full Image Segmentation, it never overlaps a mask you already placed. Reach for this to pick off cells one at a time or clean up after a full-image run.', highlightSelector: '[data-tool="sambox"]' },
