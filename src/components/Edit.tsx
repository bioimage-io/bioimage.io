...
/**
 * Max last-modified time across a model artifact's content files. Mirrors the
 * model-runner's `latest_remote_modified` (model_cache/cache.py) so the
 * test-report staleness check compares like for like. Returns null when no
 * file carries a timestamp.
 */
...
const Edit: React.FC = () => {
  ...
  // Detects whether the artifact uses 'bioimageio.yaml' (new) or 'rdf.yaml' (legacy).
  ...
  // Max last-modified time across the model's content files. This mirrors how
  // the model-runner computes the report's `latest_remote_modified` (see
  // model_cache/cache.py) so staleness is an apples-to-apples file-mtime
  // comparison. Using the artifact's `last_modified` instead would
  // false-positive, because any manifest edit bumps it.
  const [latestModelFileModified, setLatestModelFileModified] = useState<number | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Staleness is derived separately (see effect below) once both the
        // report and the current max content-file mtime are known.
      } catch {
        if (!cancelled) {
          setStoredTestReport(null);
        }
  ...
  // Derive test-report staleness: the report is outdated only when a model
  // content file changed after the test ran. Both sides are file mtimes,
  // matching the model-runner's own comparison, so manifest-only edits never
  // false-positive.
  useEffect(() => {
    const reportModified = (storedTestReport as any)?.latest_remote_modified;
    setIsTestReportOutdated(
      reportModified != null &&
      latestModelFileModified != null &&
      latestModelFileModified > (reportModified as number)
    );
  }, [storedTestReport, latestModelFileModified]);
  ...
  const handleEditorChange = (value: string | undefined, file: FileNode) => {
    ...
    // If this is the RDF spec file, mark content as changed and invalidate previous validation
  ...
  const handleSave = async (file: FileNode, contentOverride?: string, sha256Override?: { path: string, sha: string }) => {
    ...
    if (endsWithRdfFileName(file.path) && (hasContentChanged || contentOverride) && !isContentValid) {
    ...
    try {
      ...
      try {
        ...
        if (endsWithRdfFileName(file.path)) {
          ...
          try {
            ...
            if (sha256Updated) {
              ...
              setFiles(prevFiles =>
                prevFiles.map(f =>
                  endsWithRdfFileName(f.path)
                    ? { ...f, content: content, edited: true }
                    : f
                )
              );

              // If the RDF spec file is currently selected, update selectedFile to refresh editor
              setSelectedFile(prev =>
                prev && endsWithRdfFileName(prev.path)
            ...
            console.error('Error saving RDF spec file:', error);
            setUploadStatus({
              message: `Error saving ${rdfFileName}`,
              severity: 'error'
            });
            return;
          }
        } else {
          // Handle non-RDF files
          ...
          if (fileSha256) {
            ...
            // Check if the RDF spec file needs sha256 update for this file
            try {
              const rdfFile = files.find(f => endsWithRdfFileName(f.path));
              if (rdfFile) {
                // Get the current RDF spec file content
                let rdfContent: string | null = null;
                if (unsavedChanges[rdfFileName]) {
                  rdfContent = unsavedChanges[rdfFileName];
                ...
                if (rdfContent) {
                  ...
                  if (updated && newRdfContent) {
                    ...
                    // Trigger save for the RDF spec file (this will use the normal save flow)
              ...
              console.error(`Error updating sha256 in ${rdfFileName}:`, error);
            }
          }
        }
        ...
        // Refresh the max content-file mtime after a staged save: the file we
        // just wrote has a new mtime, so re-list and recompute. The derived
        // staleness effect picks this up and re-evaluates against the report.
        if (isStaged && storedTestReport) {
          try {
            const freshFiles = await artifactManager.list_files({
              artifact_id: artifactId,
              version: 'stage',
              _rkwargs: true,
            });
            setLatestModelFileModified(maxModelFileMtime(freshFiles));
          } catch {
            // Leave the current mtime/outdated flag unchanged if the re-list fails
          }
        }
  ...
  const renderFileContent = () => {
    ...
    if (endsWithRdfFileName(selectedFile.name)) {
      return (
      ...
      if (rdfFile) {
        ...
        if (!rdfFile.content) {
          ...
          if (typeof content === 'string') {
            ...
            throw new Error(`Failed to load ${rdfFileName} content`);
          }
        } else {
      ...
      try {
        ...
        try {
          ...
          if (rdfFile && fileSha256) {
            if (unsavedChanges[rdfFileName]) {
              rdfContentForPrompt = unsavedChanges[rdfFileName];
            } else if (rdfFile.content && typeof rdfFile.content === 'string') {
          ...
          console.error(`Error checking ${rdfFileName} references:`, error);
        }
          ...
          try {
            ...
            if (rdfContent) {
              ...
              if (updated) {
                ...
                // Trigger save for the RDF spec file
                const rdfFileNode: FileNode = {
                  name: rdfFileName,
                  path: rdfFileName,
                  content: newRdfContent,
  ...
  const handleValidationComplete = (result: ValidationResult) => {
    ...
    if (!result.success && selectedFile && endsWithRdfFileName(selectedFile.path)) {
    ...
    // If validation successful, check for type changes in the RDF spec file
    if (result.success) {
      const rdfFile = files.find(file => endsWithRdfFileName(file.path));
      if (rdfFile) {
        try {
          ...
          console.error(`Error parsing ${rdfFileName}:`, error);
        }
      }
    }
  };
  ...
  const renderActionButtons = () => {
    // Get the latest content for the RDF spec file, including unsaved changes
    ...
    const isRdfFile = selectedFile ? endsWithRdfFileName(selectedFile.path) : false;
  ...
  const handleValidate = () => {
    if (!selectedFile || !endsWithRdfFileName(selectedFile.path)) {
  ...
  const renderShaDialog = () => (
    ...
    >
      <div className="p-6">
        ...
        <div className="space-y-4">
          <p className="text-sm text-gray-500">
            The file content for <span className="font-semibold text-gray-700">{shaDialogState?.fileName}</span> has changed since it was last referenced in <code className="bg-gray-100 px-1 py-0.5 rounded text-xs">{rdfFileName}</code>.
          </p>
          <p className="text-sm text-gray-500">
            Do you want to update the reference (SHA256) in <code className="bg-gray-100 px-1 py-0.5 rounded text-xs">{rdfFileName}</code>?
          </p>
        </div>
        <div className="mt-6 flex gap-3 justify-end">
          <button
