...
/**
 * Max last-modified time across a model artifact's content files, excluding
 * `test_report.json`. Mirrors the model-runner's `latest_remote_modified`
 * (model_cache/cache.py) so the test-report staleness check compares like for
 * like. Returns null when no eligible file carries a timestamp.
 */
const maxModelFileMtime = (fileList: any[]): number | null => {
  const mtimes = (fileList || [])
    .filter(f => f?.type === 'file' && f?.name !== 'test_report.json' && typeof f?.last_modified === 'number')
...
const Edit: React.FC = () => {
  ...
  // Max last-modified time across the model's content files, EXCLUDING
  // test_report.json. This mirrors how the model-runner computes the
  // report's `latest_remote_modified` (see model_cache/cache.py) so staleness
  // is an apples-to-apples file-mtime comparison. Using the artifact's
  // `last_modified` instead would false-positive, because writing the report
  // back into the artifact (or any manifest edit) bumps it.
  ...
  // (test_report.json excluded), matching the model-runner's own comparison,
  // so manifest-only edits and writing the report back never false-positive.
  useEffect(() => {
  ...
  const loadArtifactFiles = async (versionOverride?: string) => {
    ...
    try {
      ...
      // Track the max content-file mtime (excluding test_report.json) so the
      // test-report staleness check compares like-for-like file mtimes.
                ...
                <div className="absolute right-2 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 flex items-center gap-1 transition-opacity">
                  ...
                  {isUnreferenced && (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        alert(`This file is not referenced in ${rdfFileName}.\n\nOnly files with a reference in the ${rdfFileName} can be visible to model test runs and other services.`);
                      }}
