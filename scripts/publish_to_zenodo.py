...
async def publish_artifact_to_zenodo(artifact_manager, artifact_summary, semaphore, index, total_count, publish_to="sandbox_zenodo", artifact_filter=None, dry_run=False):
    ...
    async with semaphore:
        try:
            ...
            if artifact_filter and artifact_filter.lower() not in artifact_alias.lower():
                ...
                return {"processed": False, "published": False, "alias": artifact_alias, "reason": "filtered"}
            ...
            if not manifest.get("name"):
                ...
                print(f"  ❌ Skipped - Missing required 'name' field")
                return {"processed": True, "published": False, "alias": artifact_alias, "reason": "missing_name"}
            
            if not manifest.get("description"):
                logger.warning(f"Skipping artifact {artifact_alias}: Missing required 'description' field")
                print(f"  ❌ Skipped - Missing required 'description' field")
                return {"processed": True, "published": False, "alias": artifact_alias, "reason": "missing_description"}
            ...
            if dry_run:
                ...
                return {"processed": True, "published": True, "alias": artifact_alias, "reason": "dry_run"}
            ...
            return {"processed": True, "published": True, "alias": artifact_alias, "record": record}
            
        except Exception as e:
            logger.error(f"Failed to publish {artifact_summary.get('alias', artifact_summary['id'])}: {e}")
            print(f"  ❌ ERROR: Failed to publish - {e}")
            return {"processed": True, "published": False, "alias": artifact_summary.get('alias', artifact_summary['id']), "error": str(e)}
