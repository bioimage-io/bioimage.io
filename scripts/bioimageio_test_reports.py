...
def is_previous_test_successful(test_results: Dict[str, Any]) -> bool:
    """Return True unless the cached test explicitly failed at service level.

    Only explicit failure statuses generated by this script should force a rerun.
    Any other status is treated as a successful service run where failures are
    model-related and can be skipped until the model changes.
    """
    status = str(test_results.get("status", "")).strip().lower()

    return status not in {"service-failed", "service-timeout"}


def get_report_bioimageio_versions(
    test_results: Dict[str, Any],
) -> Optional[Dict[str, str]]:
    """Extract bioimageio.core/spec versions from a test report env field.

    Returns None when env is missing, malformed, or incomplete.
    """
    ...
    for package in env:
        if not isinstance(package, list) or len(package) < 2:
            continue

        name = package[0]
        version = package[1]
        if isinstance(name, str) and isinstance(version, str):
            if name in {"bioimageio.core", "bioimageio.spec"}:
                versions[name] = version

    if "bioimageio.core" not in versions or "bioimageio.spec" not in versions:
        return None

    return versions


async def get_latest_change(artifact_manager, model_id: str) -> float:
    """Return the most recent modification timestamp for a model artifact."""
    files = await artifact_manager.list_files(f"bioimage-io/{model_id}")
...
def create_service_failure_result(
    model_id: str, error_msg: str, status: str
) -> Dict[str, Union[str, None, list]]:
    """Create a service-level failure result with explicit status."""
    return {
        "name": "bioimageio format validation",
        "source_name": None,
        "id": model_id,
...
async def test_bmz_models(
    ...
    service_versions_response = await runner.get_bioimageio_versions()
    service_versions = {
        "bioimageio.core": service_versions_response.get("bioimageio.core"),
        "bioimageio.spec": service_versions_response.get("bioimageio.spec"),
    }
    ...
    for model_id in model_ids:
        ...
        try:
            ...
            if test_results_file.exists():
                ...
                previous_test_time = test_results_file.stat().st_mtime
                if latest_change <= previous_test_time:
                    if not is_previous_test_successful(previous_test_results):
                        print(
                            f"-> Model '{model_id}' unchanged, but previous run had service-level failure; rerunning"
                        )
                    else:
                        previous_versions = get_report_bioimageio_versions(
                            previous_test_results
                        )
                        if previous_versions is None:
                            print(
                                f"-> Model '{model_id}' unchanged, but previous report env is missing or invalid; rerunning"
                            )
                        elif previous_versions != service_versions:
                            print(
                                f"-> Model '{model_id}' unchanged, but bioimageio versions changed; rerunning"
                            )
                        else:
                            print(
                                f"-> Model '{model_id}' unchanged with matching bioimageio versions, skipping test run"
                            )
                            continue
            ...
            test_results = create_service_failure_result(
                model_id, "Test timed out after 5 minutes", "service-timeout"
            )
        except Exception as e:
            # Parse the error message to extract clean error information
            test_results = parse_error_message(model_id, str(e))
            test_results["status"] = "service-failed"
            model_execution_time = time.time() - model_start_time
